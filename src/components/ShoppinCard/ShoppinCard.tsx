...
const ShoppingCart: React.FC = () => {
  ...
  const total = Object.values(cartFood).reduce(
  ...
  const handleCheckout = () => {
    ...
    if (token) {
      const orderData = {
        CustomerId: 0,
        BranchStoreId: 1,
        FoodIds: Object.values(cartFood).map((item) => item.Id),
        DrinkIds: Object.values(cartDrinks).map((item) => item.Id),
      };
      ...
      {Object.keys(cartFood).length === 0 ? (
        ...
        <div>
          ...
          <div className="flex justify-end mt-4">
            ...
            <button
              className="bg-blue-500 text-white px-4 py-2 mt-2 ml-4"
